...
export class GalaxyLayer {
  ...
  createParticles() {
    ...
    // Big Bang Colors (Hot to Cold) - 更多绚烂的颜色
    const bangColors = [
    ...
    for (let i = 0; i < this.config.particleCount; i++) {
      ...
      const initialColor =
        bangColors[Math.floor(Math.random() * bangColors.length)];
  ...
  animate() {
    ...
    if (this.bigBangState === "explosion") {
      ...
      const originalPositions =
        this.particlesGeometry.attributes.originalPosition.array;
      const colors = this.particlesGeometry.attributes.color.array;
      const count = this.particlesGeometry.attributes.position.count;
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (formationProgress < 1) {
          ...
          if (formationProgress > 0) {
            ...
            // Color Transition - 保持绚烂的颜色
            // 不要变成单调的青紫渐变，保持大爆炸时的多彩效果
            // 只是稍微增加亮度和饱和度
            const currentR = colors[i3];
            const currentG = colors[i3 + 1];
            const currentB = colors[i3 + 2];
            
            // 稍微增加亮度，让颜色更鲜艳
            const brightness = 1.05;
            colors[i3] = Math.min(currentR * brightness, 1);
            colors[i3 + 1] = Math.min(currentG * brightness, 1);
            colors[i3 + 2] = Math.min(currentB * brightness, 1);
          } else {
            positions[i3] = ex;
            positions[i3 + 1] = ey;
            positions[i3 + 2] = ez;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (dist < this.config.mouseInfluenceRadius) {
        const force =
          (this.config.mouseInfluenceRadius - dist) /
