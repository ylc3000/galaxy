...
export class ParticleLayer {
  ...
  init() {
    ...
    // Color cube repulsion events
    this.eventBus.on("colorCube:growing", (data) => {
      this.cubeRepulsion.active = true;
      this.cubeRepulsion.maxRadius = data.radius;
      this.cubeRepulsion.force = data.force;
    });
    ...
    this.eventBus.on("colorCube:growthComplete", () => {
      // 生长完成后保持排斥效果
      this.cubeRepulsion.radius = this.cubeRepulsion.maxRadius;
    });
  ...
  animate() {
    ...
    this.particles.forEach((p) => {
      ...
      p.draw(this.ctx, this.cubeRepulsion);
    });
  }
...
class Particle {
  ...
  update(mouse, config, cubeRepulsion) {
    ...
    // === Color Cube Repulsion (优先级最高) ===
    if (cubeRepulsion.active && cubeRepulsion.radius > 0) {
      ...
      // 扩大排斥区域以适应立方体旋转（1.3倍半径）
      const expandedRadius = cubeRepulsion.radius * 1.3;

      if (distToCenter < expandedRadius) {
        // 在扩展区域内 - 强力推开
        const angle = Math.atan2(dcy, dcx);
        // 增强推力，确保粒子被完全推出区域
        const pushForce =
          (1 - distToCenter / expandedRadius) * cubeRepulsion.force * 20;

        this.x += Math.cos(angle) * pushForce;
        this.y += Math.sin(angle) * pushForce;

        // 立方体区域内的粒子变小
        this.targetRadius = this.baseRadius * 0.2;
    ...
    if (dist < config.mouseInfluenceRadius) {
      ...
      // 1. 放大效果（基于距离的渐进式放大）
      this.targetRadius =
        this.baseRadius * (1 + influence * (config.mouseScaleMultiplier - 1));
    ...
    } else if (!cubeRepulsion.active || cubeRepulsion.radius === 0) {
      // 只有在没有立方体排斥时才恢复原始大小
      this.targetRadius = this.baseRadius;
  ...
  draw(ctx, cubeRepulsion) {
    ...
    if (cubeRepulsion.active && cubeRepulsion.radius > 0) {
      ...
      // 在立方体区域内的粒子完全不绘制
      // 使用 1.3 倍半径确保旋转时立方体对角线也不会碰到粒子
      // 立方体对角线长度 ≈ √3 ≈ 1.732，使用 1.3 提供足够缓冲
      if (distToCenter < cubeRepulsion.radius * 1.3) {
        return;
      }
    }
    ...
    if (this.glowIntensity > 0.05) {
      ...
      gradient.addColorStop(
        0,
        `hsla(${this.h}, ${this.s}%, ${this.l}%, ${
          this.glowIntensity * 0.6 * opacity
        })`
      );
      gradient.addColorStop(
        0.5,
        `hsla(${this.h}, ${this.s}%, ${this.l}%, ${
          this.glowIntensity * 0.3 * opacity
        })`
      );
    ...
    ctx.fillStyle = `hsla(${this.h}, ${this.s}%, ${adjustedL}%, ${opacity})`;
    ctx.fill();

    // 高亮边缘（增强立体感）
    if (this.glowIntensity > 0.1) {
      ctx.strokeStyle = `hsla(${this.h}, ${this.s}%, ${Math.min(
        this.l + 30,
        100
      )}%, ${this.glowIntensity * 0.5 * opacity})`;
      ctx.lineWidth = 1;
      ctx.stroke();
    }
  }
